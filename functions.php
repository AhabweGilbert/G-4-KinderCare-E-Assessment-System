	...
	echo '<div class="content home-content">';
        echo '<div class="content-text">';
               ...
               <div class="sidebar">
                    ...
                    <li class="dashitem">
						<form action="Assignments_scores.php" method="post" class="dashlink">
...
function register(){
    ...
    <table class = "clicked" id="" >
                ...
                <form action="" method="GET">
                    <div style="float: right; margin-right: 210px; padding-bottom: 5px">
                        <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>" placeholder="search pupil">
            ...
            if(isset($_GET['search']))
            {
                ...
                $sql = "SELECT UserCode, FirstName, LastName, TelephoneNo, status1 from pupils WHERE CONCAT(UserCode,FirstName,LastName) LIKE '%$filtervalues%' ORDER BY TimeStamp";
            }else{
...
//dashboard function for index page
function dashboard(){
    include('connection.php');
    ...
    echo '<h4>Assignments</h4>';
            ...
            if($result-> num_rows > 0){
               ?>
                ...
                while($row = $result-> fetch_assoc()){
                   $id=$row["AssignmentID"];
                    $query3 = "SELECT COUNT(AssignmentID) as totalattempt FROM scores WHERE AssignmentID = '$id'";
                    $attempt1 = mysqli_query($connection,$query3);
                    $attempt2=mysqli_fetch_assoc($attempt1);
                    $attempt3=$attempt2['totalattempt'];
                    $missed=$registered3-$attempt3;

                    $query4 = "SELECT AVG(score) as averagescore FROM scores WHERE AssignmentID = '$id'";
                    $avg1 = mysqli_query($connection,$query4);
                    $avg2=mysqli_fetch_assoc($avg1);
                    $avg3=$avg2['averagescore'];
                    ...
                    echo "<td>". $row["AssignmentID"]."</td>";
                    if($attempt3==0){
                        echo '<td>Not yet attempted</td></tr>';
                    }
                    else{
                        echo "<td>". $attempt3."</td>";
                        echo "<td>". $missed."</td>";
...
//function for viewing assignment scores  
function assignment_scores(){
        ?>
        <div class="b" id = "">
         ...
         <?php
            ...
            if($result-> num_rows > 0){
                ...
                while($row = $result-> fetch_assoc()){
                    ...
                    echo "<td>";
                    ?>
                    <input type = "text" name="UserID" value="<?php echo $row['UserCode']; ?>" readonly />
                    <?php
                    echo "</td>";
                    echo "<td>";
                    ?>
                      <input type = "text" name="AssignmentID" value="<?php echo $row['AssignmentID']; ?>" readonly />
                    <?php
                    echo "</td>";
                    echo "<td>";
                    ?>
                     <input type = "text" name="score" value="<?php echo $row['score']; ?>" readonly />
                    <?php
                    echo "</td>";
                    if($row['comment']!=NULL){
                        echo "<td>";
                        ...
                        <?php
                        echo "</td>";
                        echo "<td>".'<button type = "submit" name="">Edit Comment</button>'."</td>";
                    }
                    else{
                        echo "<td>";
                        ?>
                        ...
                        <?php
                        echo "</td>";
                        echo "<td>".'<button type = "submit" name="">Forward Comment</button>'."</td>";
                    }
                    echo "</tr>";
                    echo '</form>';
                }
